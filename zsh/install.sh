...
# Atuin's behavior config (search defaults, sync on/off, UX) is tracked
# in zsh/atuin-config.toml. Atuin's machine-local state (SQLite DB,
# encryption key, session) lives in ~/.local/share/atuin/ and is NOT
# touched here.
if command -v atuin >/dev/null 2>&1; then
    mkdir -p "$HOME/.config/atuin"
    # If a non-symlink config already exists (auto-generated by atuin
    # on first run, or hand-edited), back it up before replacing — so a
    # fresh symlink doesn't clobber anything machine-specific.
    if [ -f "$HOME/.config/atuin/config.toml" ] && [ ! -L "$HOME/.config/atuin/config.toml" ]; then
        mv "$HOME/.config/atuin/config.toml" "$HOME/.config/atuin/config.toml.pre-dotfiles"
